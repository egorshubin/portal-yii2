...
if ($attributes['status_id'] == 1) {
    $publish = Html::a('<i class="status-icon fa unpublish fa-toggle-on blue" aria-hidden="true"></i>', ['unpublishfromcat', 'id' => $model['id'], 'redirect' => '/admin/category/update', 'categoryId' => $categoryId, 'tableName' => $attributes['type']], [
    ...
    $publish = Html::a('<i class="status-icon fa publish fa-toggle-off red" aria-hidden="true"></i>', ['publishfromcat', 'id' => $model['id'], 'redirect' => '/admin/category/update', 'categoryId' => $categoryId, 'tableName' => $attributes['type']], [
        'title' => 'Опубликовать',
        'data' => [
            'method' => 'post',
        ],
